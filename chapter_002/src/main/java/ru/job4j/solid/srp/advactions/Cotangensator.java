 ...
 * @since 04.06.2019
 */
public class Cotangensator implements UserAction {
    ...
    /**
     * Метод выполняет вычисление котангенса.
     *
    ...
    private double getOperand(int select) {
        ...
        if (select != 2) {
            value = input.askOperand("Input angle value:");
        }
        return value;
    }
    ...
    @Override
    public String info() {
        return String.format("%s. %s", this.key(), "Get cotan.");
    }
}
